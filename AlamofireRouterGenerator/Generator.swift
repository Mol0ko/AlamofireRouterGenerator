...
class Generator {
    ...
    func generateSwift3Router() -> String? {
        ...
        let folderedRequests = recognizeFolderedRequests(folders: folders, allRequests: allRequests)
        
        // 2 - generate requests with parameters:       (first part of router enum)
        let requestsNamesSection = generateRequestsNamesWithParameters(folders: folderedRequests)
    ...
    private func recognizeFolderedRequests(folders: [ItemResponse], allRequests: [ItemResponse]) -> [FolderWithRequests] {
        var requests = [FolderWithRequests]()
        ...
        if folders.count > 0 {
            folders.forEach {
                ...
                requests.append(FolderWithRequests(folderName: folder.name ?? "", requestsInFolder: requestsInFolder))
            }
        } else {
            let rootFolder = FolderWithRequests(folderName: "root", requestsInFolder: allRequests)
            requests.append(rootFolder)
        }
        return requests
    }
    ...
    private func generateRequestsNamesWithParameters(folders: [FolderWithRequests]) -> String {
        var requestsString = "\n\n"
    ...
    public func generateasUrlRequestFuncSection(allRequests: [ItemResponse]) -> String {
        let asUrlRequestFields = "\t\tlet url = try baseURL.asURL()\n\t\tvar urlRequest = URLRequest(url: url.appendingPathComponent(path))\n\t\turlRequest.httpMethod = method.rawValue\n\t\turlRequest.setValue(\"application/json\", forHTTPHeaderField: \"Content-Type\")"
        var cases = ""
